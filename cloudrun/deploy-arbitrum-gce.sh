...
VM_NAME="${VM_NAME:-rpc-lb-arbitrum}"
IMAGE_FAMILY="ubuntu-2204-lts"
IMAGE_PROJECT="ubuntu-os-cloud"
MACHINE_TYPE="e2-micro"

# Gateway endpoints
ACTIVE_GATEWAY="${ACTIVE_GATEWAY:-http://34.126.125.90:3849}"
PASSIVE_GATEWAY="${PASSIVE_GATEWAY:-http://34.126.177.240:3849}"

# External fallback endpoints
EXTERNAL_RPC_1="${EXTERNAL_RPC_1:-https://arb1.arbitrum.io/rpc}"
EXTERNAL_RPC_2="${EXTERNAL_RPC_2:-https://arbitrum-one.publicnode.com}"
EXTERNAL_RPC_3="${EXTERNAL_RPC_3:-https://rpc.ankr.com/arbitrum}"

PORT=8080

echo "=== Deploying Arbitrum One Load Balancer to GCE ==="
...
gcloud compute ssh "$VM_NAME" --zone="$ZONE" --project="$PROJECT_ID" --command="
    sudo tee /etc/systemd/system/arbitrum-lb.service > /dev/null <<'EOF'
[Unit]
Description=Arbitrum One RPC Load Balancer
...
Environment=EXTERNAL_RPC_1=$EXTERNAL_RPC_1
Environment=EXTERNAL_RPC_2=$EXTERNAL_RPC_2
Environment=EXTERNAL_RPC_3=$EXTERNAL_RPC_3
ExecStart=/usr/bin/node /opt/load-balancer/dist/cloudrun/arbitrum-gateway-cloudrun.js
Restart=always
RestartSec=10
StandardOutput=journal
StandardError=journal
SyslogIdentifier=arbitrum-lb
...
EOF
    ...
    sudo systemctl enable arbitrum-lb
    sudo systemctl restart arbitrum-lb
    sleep 5
    sudo systemctl status arbitrum-lb --no-pager
"
...
echo ""
echo "View logs:"
echo "  gcloud compute ssh $VM_NAME --zone=$ZONE --project=$PROJECT_ID --command='sudo journalctl -u arbitrum-lb -f'"
echo ""
