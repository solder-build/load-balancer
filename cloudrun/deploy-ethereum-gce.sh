...
VM_NAME="${VM_NAME:-rpc-lb-ethereum}"
IMAGE_FAMILY="ubuntu-2204-lts"
IMAGE_PROJECT="ubuntu-os-cloud"
MACHINE_TYPE="e2-micro"

# Gateway endpoints
ACTIVE_GATEWAY="${ACTIVE_GATEWAY:-http://34.126.125.90:3850}"
PASSIVE_GATEWAY="${PASSIVE_GATEWAY:-http://34.126.177.240:3850}"

# External fallback endpoints
EXTERNAL_RPC_1="${EXTERNAL_RPC_1:-https://eth.llamarpc.com}"
EXTERNAL_RPC_2="${EXTERNAL_RPC_2:-https://ethereum.publicnode.com}"
EXTERNAL_RPC_3="${EXTERNAL_RPC_3:-https://1rpc.io/eth}"

PORT=8080

echo "=== Deploying Ethereum Load Balancer to GCE ==="
...
gcloud compute ssh "$VM_NAME" --zone="$ZONE" --project="$PROJECT_ID" --command="
    sudo tee /etc/systemd/system/ethereum-lb.service > /dev/null <<'EOF'
[Unit]
Description=Ethereum RPC Load Balancer
...
ExecStart=/usr/bin/node /opt/load-balancer/dist/cloudrun/ethereum-gateway-cloudrun.js
Restart=always
RestartSec=10
StandardOutput=journal
StandardError=journal
SyslogIdentifier=ethereum-lb
...
EOF
    ...
    sudo systemctl enable ethereum-lb
    sudo systemctl restart ethereum-lb
    sleep 5
    sudo systemctl status ethereum-lb --no-pager
"
...
echo ""
echo "View logs:"
echo "  gcloud compute ssh $VM_NAME --zone=$ZONE --project=$PROJECT_ID --command='sudo journalctl -u ethereum-lb -f'"
echo ""
